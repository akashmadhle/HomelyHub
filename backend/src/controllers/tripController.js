...
// Send both API trip plan + matching properties back to the frontend / user
...
const createTripPlan = async(req,res) => {
    try{
        ...
        res.status(500).json({
            status:"fail",
            messsage:"Could Not create a trip plan , Please Try Again"
...
const writeDescription = async(req,res) => {
   try{
    ...
    res.status(500).json({
        status:"fail",
        message:"Could not Generate a Description, Please Tru Again Later"
    })
